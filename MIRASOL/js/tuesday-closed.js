...
(function () {
  ...
  function buildTuesdayPopup() {
    ...
    modal.innerHTML =
      ...
      `<img src="${escapeHtml(logo)}" alt="${escapeHtml(brandName)}" class="tuesday-closed-modal__logo" width="84" height="48">` +
      `</div></div>` +
      ...
      `</header>` +
      `<div class="tuesday-closed-modal__divider" aria-hidden="true"><span class="tuesday-closed-modal__divider-gem"></span></div>` +
      ...
      `</div></div>` +
      `<div class="tuesday-closed-modal__craft" aria-hidden="true"></div>` +
      `</div>`;
