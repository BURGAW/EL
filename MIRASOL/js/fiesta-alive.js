...
(function () {
  ...
  function initPickupEstimator() {
    ...
    let mins = 15;
    if (h >= 11 && h < 14) mins = 22;
    if (h >= 17 && h < 20) mins = 28;

    const el = document.createElement('a');
    el.className = 'fiesta-pickup';
    el.id = 'fiesta-pickup';
    el.href = 'tel:9107891154';
    el.setAttribute('aria-label', `Call now for pickup in about ${mins} minutes`);
    el.innerHTML =
      ...
      `</span>` +
      `<span class="fiesta-pickup__eta">Pick up in ~<strong class="fiesta-pickup__time">${mins} min</strong></span>`;
    hero.after(el);
  }
